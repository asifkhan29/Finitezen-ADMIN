    ...
    <div className="min-h-screen grid place-items-center px-4 relative overflow-hidden bg-background">
      ...
      <div className="w-full max-w-sm rounded-2xl border bg-card p-8 shadow-[var(--shadow-card)] text-center flex flex-col items-center">
        ...
        <div className="w-full space-y-4 my-4">
          <h1 className="text-lg font-semibold tracking-tight text-left">Verifying Session</h1>
          <p className="text-xs text-muted-foreground text-left mt-1">Establishing link with the security workspace cluster.</p>
          ...
          <div 
            ...
            style={{ background: "var(--gradient-brand)" }}
          >
            <Loader2 className="h-4 w-4 animate-spin" /> Authorizing Access Node...
          </div>
        </div>
      </div>
    </div>
  ...
  useEffect(() => {
    const checkTokenStatus = async () => {
      ...
      try {
        ...
        if (statusText === "true") {
          setTimeout(() => setIsValid(true), 600);
        } else {
